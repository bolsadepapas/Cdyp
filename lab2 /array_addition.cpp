...
cl::Device
getDefaultDevice(); // Return the first device found in this OpenCL platform.
void initializeDevice(); // Inicialize device and compile kernel code.
void seqSumArrays(int *a, int *b, int *c,
                  const int N); // Sequentially performs the N-dimensional
                                // operation c = a + b.
void parSumArrays(
    int *a, int *b, int *c,
    const int N); // Parallelly performs the N-dimensional operation c = a + b.
bool checkEquality(
    int *c1, int *c2,
    const int N); // Check if the N-dimensional arrays c1 and c2 are equal.
...
void parSumArrays(int *a, int *b, int *c, const int N) {
  ...
  cl::Buffer bBuf(
      context, CL_MEM_READ_ONLY | CL_MEM_HOST_NO_ACCESS | CL_MEM_COPY_HOST_PTR,
      N * sizeof(int), b);
  cl::Buffer cBuf(context, CL_MEM_WRITE_ONLY | CL_MEM_HOST_READ_ONLY,
                  N * sizeof(int));
